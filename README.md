...
- **更新**: GitHub Actions 毎朝 07:00 JST + 3時間ごと差分取得
...
```
GitHub Actions
  ├─ daily-update.yml: 22:00 UTC (07:00 JST) + 3時間ごと差分
  └─ manual-update.yml: 手動で日付範囲を指定したバックフィル
     │  azure/login@v2 (OIDC)
     ▼
┌──────────────────────┐
│ src/fetch_feeds.py   │  RSS 13ソース取得 + 重複除外（通常フロー）
...
| File | Purpose |
|---|---|
| `data/raw_articles.json` | RSS から取得した新着・手動範囲指定の記事を、LLM 処理前に一時保存 |
| `data/articles.json` | LLM 分類・要約に成功した記事の永続データ。サイト生成の source of truth |
| `data/seen_urls.json` | LLM 分類・要約に成功済みの URL 一覧。次回以降の重複除外に使用 |
...
→ GitHub Actions: Actions タブ → 「Manual Update」→ Run workflow で `start_date` / `end_date` を入力。
...
├── .github/workflows/
│   ├── daily-update.yml                 # メイン自動更新（07:00 JST + 3h差分）
